...
class Aggregator:
    def __init__(self,feeds,interval):
        ...
        self.uniq_indicators_of_all_feeds = [] #memory can be optimized: instead of duplicating all uniq indicators (from feed's indicator), consider saving only pointers
        self.interval = interval #in seconds


    def receive_indicators_from_all_feeds(self):
        self.uniq_indicators_of_all_feeds = []
        for f in self.list_of_feeds:
            f.receive_indicators_from_feed()
            for ind in f.get_indicators():
                # Make sure the same indicator was not already added
                # but cannot use: "if ind not in self.uniq_indicators_of_all_feeds:" since need to compare only security related info
                if len(list(filter(ind.is_same_indicator, self.uniq_indicators_of_all_feeds))) == 0: #performance can be optimized: consider breaking when found one match, no need to check further values
                    self.uniq_indicators_of_all_feeds.append(ind)
        print ("Done getting all indicators. Got", len(self.uniq_indicators_of_all_feeds), "uniq indicators")


    def get_uniq_indicators_from_all_feeds(self):
        return self.uniq_indicators_of_all_feeds


    def generate_security_rules(self):
        print("Generating security policy.... TBD, for example:")
        for ind in self.get_uniq_indicators_from_all_feeds():
            print (ind.get_five_tuple() + ind.get_signature())


    def install_feeds_policy(self):
        print ("Installing feed policy from", len(self.uniq_indicators_of_all_feeds), "indicators.... TBD")
...
class Feed:
    def __init__(self,name,link):
        self.id = hashlib.sha224((link+name).encode('utf-8')).hexdigest() #TODO: generate a shorter ID
        self.name = name
        self.link = link
    ...
    def receive_indicators_from_feed(self):
        ...
        for entry in feed_response.entries:
            indicator = Indicator(entry,self.name)
...
class Indicator:
    def __init__(self,rss_entry,feed_name):
        ...
        # The 5-tuple data and the MD5 are currently generated by manipulating 'published' value.
...
def init_feeds_and_aggregator():
    ...
    return Aggregator(tmp_feeds,interval)
